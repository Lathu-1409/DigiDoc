...
MongoClient client;
    ...
    public void mongoClient() {
        ...
        String connectionString = "mongodb+srv://user:password@cluster.hvufz.mongodb.net/?retryWrites=true&w=majority&appName=VickyDevCluster";
    ...
    public MongoCollection<User> getUserCollection() {       
        MongoDatabase database = client.getDatabase("ResourseAccess")
                .withCodecRegistry(fromRegistries(
                        MongoClientSettings.getDefaultCodecRegistry(),
                        fromProviders(PojoCodecProvider.builder().automatic(true).build())
                ));

        return database.getCollection("users", User.class);  // Use the User class for the collection
    }
}