document.getElementById("mostDownloadedForm").addEventListener("submit", async function (event) {
  event.preventDefault();

  const count = document.getElementById("count").value;

  try {
      const response = await fetch(`/documents/most-downloaded/${count}`);
      if (response.ok) {
          ...
          alert("Error fetching most downloaded documents: " + error);
      }
  } catch (err) {
