document.getElementById("recentForm").addEventListener("submit", async function (event) {
  event.preventDefault();

  const count = document.getElementById("count").value;

  try {
      const response = await fetch(`/documents/recent/${count}`);
      if (response.ok) {
          ...
          alert("Error fetching recently uploaded documents: " + error);
      }
  } catch (err) {
